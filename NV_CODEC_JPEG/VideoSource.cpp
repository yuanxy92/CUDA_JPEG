...
VideoSource::VideoSource(const std::string sFileName, FrameQueue *pFrameQueue, int nBitDepth): hVideoSource_(0), nBitDepth(nBitDepth)
{
...
const
{
    ...
    if (nBitDepth) {
        oFormat.bit_depth_luma_minus8 = nBitDepth - 8;
        oFormat.bit_depth_chroma_minus8 = nBitDepth - 8;
    }
    return oFormat;
}
...
operator << (std::ostream &rOutputStream, const CUVIDEOFORMAT &rCudaVideoFormat)
{
    ...
    rOutputStream << "\tBit depth       : " << rCudaVideoFormat.bit_depth_luma_minus8 + 8 << "\n";
